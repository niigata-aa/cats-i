...
<div class="A">
	<form action = "updateKeeper" method="post" enctype="multipart/form-data" >
		...
		<input type = "radio" name="workingNow" value=0 <%if(updateEmp.getWorkingNow() ==0){ %> checked=checked <%} %>>退籍<br>
	
		　　　飼育員ID : <input type="text" name="empID" class="text" pattern="[0-9]{6}" required
				   value=<%=updateEmp.getEmpID() %> placeholder="6桁の数字で入力してください"><br>
			   
		　担当動物種族 : <input type="text" name="" class="text"
				   value=><br>

		　　　　　名前 : <input type="text" name="lastName" class="text" required
				   value=<%= updateEmp.getLastName() %>>
			   
				<input type="text" name="firstName" class="text" required
				   value=<%= updateEmp.getFirstName() %>><br>
		　　　　　性別 : <select name="gender" class="select">
		...
		　　担当エリア : <select name="area" class="select">
